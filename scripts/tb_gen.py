...
e.g.:
    ./tb_gen.py --num-tests 500 --operation ppu -n 16 -es 1 --no-shuffle-random
"""
...
def single_arg_func(c, op_i):
  c += "`ifdef FLOAT_TO_POSIT\n"
  c += f"out_ground_truth = 'bz;\n"
  if op_i == Tb.FLOAT_TO_POSIT:
    c += f"op_i = {op_i.name};\n"
    c += f'op_i_ascii = "{op_i.name}";\n\n'
...
def func(c, op_i, list_a, list_b):
  ...
  for counter, (a, b) in enumerate(zip(list_a, list_b)):
    ...
    c += f"""{'operand2_i_ascii ='.ljust(LJUST)} "{p2.eval()}";\n\t"""
    c += f"""{'result_gt_ascii ='.ljust(LJUST)} "{pout.eval()}";\n\t"""

    if PIPELINED:
      # adding N_CYCLES_DEL cycle delay
      if op_i == Tb.PACOGEN:
        pass
      elif op_i == Tb.ADD or op_i == Tb.SUB or op_i == Tb.MUL or op_i == Tb.DIV:
        c += f"#{10 * 3};\n\t"
      else:
        raise Exception()
    ...
    if op_i != Tb.DIV and op_i != Tb.PACOGEN:
