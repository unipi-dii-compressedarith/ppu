...
F ?= 0
WORD ?= 32
PPU_TOP_NAME := ppu$(WORD)_P$(N)E$(ES)_top
...
PIPELINE_STAGE := #-DPIPELINE_STAGE
...
SRC_PPU_CORE_OPS := \
	...
	$(SRC_DIR)/core_ops/lut_reciprocate.sv \
...
pickle:
	...
	$(SRC_DIR)/ppu_top.sv \
	-o $(PPU_TOP_NAME).sv && \
	iverilog -g2012 -o $(PPU_TOP_NAME).out $(PPU_TOP_NAME).sv && \
	vvp $(PPU_TOP_NAME).out -l $(PPU_TOP_NAME).log && \
	cp -r $(PPU_TOP_NAME).sv $(VIVADO_DIR)/$(PPU_TOP_NAME).sv && \
	cp -r $(PPU_TOP_NAME).sv $(QUARTUS_DIR)/ppu_top.sv && \
	make lint
...
ppu-core_ops:
	...
	$(SRC_PPU_CORE_OPS) && \
	sleep 1 && \
	vvp -l ppu_core_ops_P$(N)E$(ES).log ppu_core_ops_P$(N)E$(ES).out
...
ppu: gen-lut-reciprocate-mant verilog-quartus
	...
	$(SRC_PPU_CORE_OPS) && \
	sleep 1 && \
	vvp ppu_P$(N)E$(ES).out
	verible-verilog-format --inplace --indentation_spaces 2 $(QUARTUS_DIR)/$(PPU_TOP_NAME).sv || true
	make lint || true
...
tb_pipelined_long_tb_single_file:
	...
	sv2v -DTB_PIPELINED -DWORD=$(WORD) -DF=$(F) -DN=$(N) -DES=$(ES) $(SRC_DIR)/include/utils.sv tb_pipelined_tmp_copy.sv $(QUARTUS_DIR)/$(PPU_TOP_NAME).sv -w tb_pipelined_long_tb_single_file.sv && \
...
ppu_P8E0:
	make ppu N=8 ES=0 WORD=32 DIV_WITH_LUT=0

ppu_P16E1:
	make ppu N=16 ES=1 WORD=32 DIV_WITH_LUT=0

ppu_P32E2:
	make ppu N=32 ES=2 WORD=32 DIV_WITH_LUT=0
...
# 	$(SRC_POSIT_TO_FLOAT) \
# 	-w posit_to_float.v && cp posit_to_float.v ppu.v

# conversions-verilog-float-to-posit-quartus:
# 	cd $(QUARTUS_DIR) && \
# 	sv2v -DN=$(N) $(ES_FIELD_PRESENCE_FLAG) -DES=$(ES) -DF=$(F) \
# 	$(SRC_FLOAT_TO_POSIT) \
# 	-w float_to_posit.v && cp float_to_posit.v ppu.v
...
verilog-quartus: pickle
	sv2v $(PPU_TOP_NAME).sv -w $(PPU_TOP_NAME).v && \
	iverilog -g2012 $(PPU_TOP_NAME).v -o $(PPU_TOP_NAME).out && \
	vvp $(PPU_TOP_NAME).out -l $(PPU_TOP_NAME).log && \
	cp -r $(PPU_TOP_NAME).v $(VIVADO_DIR)/$(PPU_TOP_NAME).v && \
	cp -r $(PPU_TOP_NAME).v $(QUARTUS_DIR)/ppu_top.v && \
	make lint
...
lint:
	...
	slang $(PPU_TOP_NAME).sv # --top ppu_top
...
clean:
	rm -rf *.out a.* sources.json ppu*_top.*v tb*.vcd
	rm -rf $(WAVEFORMS_DIR)/*.out
	make -C fpga/quartus clean
	make -C tests clean
